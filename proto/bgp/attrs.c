...
{
  const struct ea_def *def = ea_def_find(a->id);

  if ((def < &bgp_attr_table[0].def) || (def >= &bgp_attr_table[BGP_ATTR_MAX].def))
    return NULL;

  return (const union bgp_attr_desc *) def;
}

#define BGP_EA_ID(code)	(bgp_attr_table[code].id)
#define EA_BGP_ID(code)	(((union bgp_attr_desc *) ea_def_find(code)) - bgp_attr_table)
...
{
  const union bgp_attr_desc *desc = &bgp_attr_table[code];
  ea_unset_attr(to, pool, 0, &desc->def);
}
...
bgp_decode_unknown(struct bgp_parse_state *s, uint code, uint flags, byte *data, uint len, ea_list **to)
{
  ...
  ea_set_attr_data(to, s->pool, &bgp_attr_table[code].def, flags, data, len);
}
...
static void
bgp_register_unknown_attr(uint code)
{
  bgp_attr_table[code] = (union bgp_attr_desc) {
    .name = mb_sprintf(&root_pool, "bgp_unknown_0x%02x", code),
    .type = EAF_TYPE_OPAQUE,
    .flags = BAF_OPTIONAL,
    .readonly = 1,
  ...
  ea_register(&bgp_attr_table[code].def);
}
...
{
#define BRA(X)	ea_register(&bgp_attr_table[X].def);
  MACRO_FOREACH(BRA,
      BA_ORIGIN,
      BA_AS_PATH,
      BA_NEXT_HOP,
...
bgp_decode_attr(struct bgp_parse_state *s, byte code, byte flags, byte *data, uint len, ea_list **to)
{
  ...
  if (!bgp_attr_table[code].id)
    bgp_register_unknown_attr(code);
